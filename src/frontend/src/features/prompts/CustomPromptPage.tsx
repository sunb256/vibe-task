...
function filterPrompts(prompts: PromptSummary[], searchQuery: string) {
  ...
  return prompts.filter((prompt) => promptMatchesQuery(prompt, query));
}

function promptMatchesQuery(prompt: PromptSummary, query: string) {
  return (
    prompt.name.toLowerCase().includes(query) ||
    prompt.path.toLowerCase().includes(query) ||
    displayPath(prompt.path).toLowerCase().includes(query)
  );
}
